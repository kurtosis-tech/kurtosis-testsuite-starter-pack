...
* String logLevelStr -  The testsuite log level string passed in at runtime, which should be parsed so that the logging framework can be configured.

### parseParamsAndCreateSuite(paramsJsonStr) -> testsuite 
...
* String paramsJsonStr -  The JSON-serialized custom params data that was passed in when Kurtosis was run, and is used to customize the testsuite's behaviour.

**Returns**

* [TestSuite][testsuite] testsuite - An instance of the user's custom [TestSuite][testsuite] implementation.
...
### addServiceToPartition(serviceId, partitionId, initializer) -> (service, checker)
...
* String serviceId -  The ID that the new service should have.
* String partitionId -  The ID of the partition that the new service should be started in. This can be left blank to start the service in the default partition if it exists (i.e. if the network hasn't been repartitioned and the default partition removed).
* [DockerContainerInitializer\<S\>][dockercontainerinitializer] initializer -  The initializer that provides the logic for starting the container for the new service.

**Returns**

* S service -  The [Service][service] implementation representing the new service, as created by the initializer's [DockerContainerInitializer.getService][dockercontainerinitializer_getservice] method.
* [AvailabilityChecker][availabilitychecker] checker -  A class for checking if the returned service is available yet, as defined by [Service.isAvailable][service_isavailable]. 
